...
*Setup instructions for a Gitian build of Testcoin using a Debian VM or physical system.*
...
- [Building Testcoin](#building-testcoin)
...
Clone the git repositories for testcoin and Gitian and then checkout the testcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the testcoin repository
...
To build Testcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the testcoin repository.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/testcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/testcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/testcoin-build.assert
```
...
[testcoin/gitian.sigs](https://github.com/testcoin/gitian.sigs/) repository, or if that's not possible create a pull
