...
export TESTCOINCLI=${BUILDDIR}/qa/pull-tester/run-testcoin-cli
export TESTCOIND=${REAL_TESTCOIND}

#Run the tests

if [ "x${ENABLE_TESTCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and testcoind must all be enabled"
fi
