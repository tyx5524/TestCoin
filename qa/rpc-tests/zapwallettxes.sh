...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars TESTCOIND and TESTCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

TESTCOIND=${TESTCOIND:-${1}/testcoind}
CLI=${TESTCOINCLI:-${1}/testcoin-cli}
...
$TESTCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$TESTCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
